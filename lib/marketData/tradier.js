...
class Tradier {
  ...
  getQuote (symbols, options = {}) {
    ...
    return this.post('markets/quotes', params).then(
      ({
        data: {
          quotes: { quote }
        }
  ...
  getHistoricalQuote (symbol, options = {}) {
    const params = {
      symbol: symbol,
      ...options
    }

    return this.get('markets/history', params).then(
      ({
        data: {
          history: { day }
        }
...
const getQuote = async (sym) => {
  try {
    let data = await tradier.getQuote(sym, { greeks: true })
    ...
    const returnData = data.map(data => {
      const change = parseFloat(data.last) - parseFloat(data.open)
      const changePercentage = change / parseFloat(data.open)
      ...
      const quoteData = {
        ...
        lastTimestamp: data.trade_date
      }

      if (data.type === 'option') {
        quoteData.contractType = data.expiration_type
        quoteData.openInterest = data.open_interest
        quoteData.greeks = data.greeks
      }
...
const getHistoricalQuote = async (sym, options = {}) => {
  try {
    const data = await tradier.getHistoricalQuote(sym, options)
