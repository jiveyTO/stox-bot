...
const listTrades = async (interaction, tradesTable, client) => {
  ...
  // default is options expiring in next 2 weeks
  if (Object.keys(args).length === 0) {
    whereClause.expiry = {
      [Op.lt]: new Date(Date.now() + 2 * 7 * 24 * 60 * 60 * 1000),
  ...
  // closed is determined later after the tally
  if (args.status) {
  ...
  if (args.status && args.status !== 'expired') {
    tradeListArr = tradeListArr.map(t => {
      ...
      if (args.status === 'closed') return (tr.openQty + tr.closeQty === 0) ? t : null
      else if (args.status === 'open') return (tr.openQty + tr.closeQty !== 0) ? t : null
  ...
  const tradeListStr = tradeListArr.map((thisTrade, i) => {
    ...
    if (tt.action === 'BTO' || tt.action === 'STO') {
      if (closedQty === tt.quantity) {
        return `--- ${preStr}${tt.tradeStr} [Closed] ${postStr}`
      } else {
        ...
        dollarReturn = (tt.action === 'BTO') ? totalAmt - initAmt : initAmt - totalAmt
      }
    } else if (tt.action === 'BTC' || tt.action === 'STC') {
    ...
    if (calcReturn > 0) {
      return `+ ${preStr}${tt.tradeStr} [${calcReturn}%|$${dollarReturn}] ${postStr}`
    } else if (calcReturn < 0) {
      return `- ${preStr}${tt.tradeStr} [${calcReturn}%|-$${dollarReturn * -1}] ${postStr}`
    } else {
      return `${preStr}${tt.tradeStr} [${calcReturn}%|$${dollarReturn}] ${postStr}`
    }
  })
  ...
  if (initialCommand.length === 1) initialCommand.push('range:+2 weeks [default]')
